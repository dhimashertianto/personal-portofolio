...
const {
  portfolio_content: { title, subtitle, portfolio_item, portfolio_filter },
} = constants;
  ...
  <div className={styles.portfolio__filter}>
    {portfolio_filter.map((item) => (
      <div
        ...
        key={item.id}
        className={`${styles.portfolio__filter__item} ${
          selectedFilter === item.id
            ? styles.portfolio__filter__item__selected
            : ""
        }`}
      >
        {item.title}
...
const renderWorkItem = (
  { setShowProjectDetail, setSelectedProject },
  selectedFilter
) => (
  <div className={`${styles.portfolio__container} container grid`}>
    {portfolio_item
      .filter((item) => item.filterId === selectedFilter)
      .map((item) => (
        <div className={styles.portfolio__card} key={item.id}>
          ...
          <div>
            ...
            {item.position !== null && (
              <h5 className={styles.portfolio__title}>{item.position}</h5>
            )}
            <a
            ...
            >
              Detail{" "}
              <i
                className={`bx bx-right-arrow-alt ${styles.portfolio__button__icon}`}
              />
            </a>
          </div>
        </div>
